...
{
    if (status == Message::Status::Ok) { ss << "ok"; }
    else if (status == Message::Status::Error) { ss << "ko"; }
    return ss;
}
...
inline std::istream& operator>>(std::istream& ss, Message::Status& status)
{
    std::string str;
    ss >> str;
    if (str == "ok") { status = Message::Status::Ok; }
    else if (str == "ko") { status = Message::Status::Error; }
    return ss;
}
