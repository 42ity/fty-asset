 ...
 */
static bool is_powering_other_rack(const ShortAssetInfo& device,
 ...
 */
static void update_border_devices(const std::map<uint32_t, ShortAssetInfo>& container_devices,
    const std::set<std::pair<uint32_t, uint32_t>>& links, std::set<ShortAssetInfo>& border_devices)
{
 ...
 *          list is empty.
 */
static std::vector<std::string> total_power_v2(const std::map<uint32_t, ShortAssetInfo>& devices_in_container,
    const std::set<std::pair<uint32_t, uint32_t>>&                                       links)
{
    ...
    for (auto& oneLink : links) {
        ...
        {
            border_devices.insert(devices_in_container.find(oneLink.second)->second);
        }
 ...
 */
static int select_total_power_by_id(tntdb::Connection& conn, uint32_t assetId, std::vector<std::string>& powerDevices)
{
    ...
    std::function<void(const tntdb::Row&)> func = [&container_devices](const tntdb::Row& row) {
        ...
        if (type_id == persist::asset_type::DEVICE) {
            ...
            container_devices.emplace(asset_id, ShortAssetInfo(asset_id, device_name, subtype_id));
        }
    };
    ...
    if (rv != 0) {
        log_warning("asset_id='%" PRIu32 "': problems appeared in selecting devices", assetId);
    ...
    if (ret_active.status == 0) {
        log_warning("asset_id='%" PRIu32 "': internal problems in links detecting", assetId);
        // so return an empty set of power devices
        powerDevices = {};
        return -1;
    }
...
int select_devices_total_power(const std::string& assetName, std::vector<std::string>& powerDevices, bool test)
{
    if (test)
        return 0;
...
void total_power_test(bool /*verbose*/)
{
    printf(" * total_power: ");
    printf("OK\n");
}
